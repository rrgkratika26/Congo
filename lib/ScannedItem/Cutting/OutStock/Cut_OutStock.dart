...
class _CuttingOutStockFormState extends State<CuttingOutStockForm> {
  ...
  void initState() {
    ...
    _avgWeightMtrCtrl = TextEditingController(text: '0.00');
    ...
    _netWtCtrl.addListener(_calculateUseAndFinalRem); // ADD THIS
    _wastageCtrl.addListener(_calculateUseAndFinalRem);
    _grossWeightCtrl.addListener(_calculateRollWeight);
    _tareWeightCtrl.addListener(_calculateRollWeight);
    _rollLengthCtrl.addListener(_calculateAvgWeight); // ← ADD
    _loadOperators();
    _loadSupervisors();
    _loadFabricWidth();
    _loadGsm();
