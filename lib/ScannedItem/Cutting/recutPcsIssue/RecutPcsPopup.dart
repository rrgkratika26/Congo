...
//
//       try {
//         woList = await JblApiService.getWorkOrders();
//       } catch (e) {
...
//           isLoadingIds = false;
//         });
//         loadWorkOrders(setState); // ← auto-trigger work orders immediately
//       } catch (e) {
//         setState(() {
//           isLoadingIds = false;
//           fetchError = e.toString();
...
//       try {
//         final url = Uri.parse(
//           'http://190.92.175.47:80/JblAPI/api/Cutting/ComponentInCuttingIssued?woNumber=${Uri.encodeComponent(woNumber)}',
//         );
...
//                                         )
//                                         .toList(),
//                                     onChanged: woList.isEmpty
//                                         ? null
//                                         : (val) {
//                                             setState(() => selectedWo = val);
//                                             if (val != null)
...
class AddRecutPcsPopup {
  static Future<void> show(
    ...
    final Map<String, List<String>> componentCache = {};

    Future<void> fetchInitialData(StateSetter setState) async {
      setState(() => isLoading = true);

      try {
        final results = await Future.wait([
          JblApiService.getNextRollEntryId(),
          JblApiService.getWorkOrders(),
        ]);

        setState(() {
          selectedId = results[0] as int;
          woList = results[1] as List<String>;
        });
      } catch (e) {
        fetchError = e.toString();
      }
    ...
    Future<void> fetchComponents(StateSetter setState, String woNumber) async {
      ...
      if (componentCache.containsKey(woNumber)) {
        setState(() {
          components = componentCache[woNumber]!;
        });
        return;
      }
      ...
      try {
        ...
        if (res.statusCode == 200) {
          ...
          components = data.map<String>((e) => e['component'].toString()).toList();

          componentCache[woNumber] = components;
        }
      } catch (_) {}
      ...
      builder: (context) => Dialog(
        ...
        child: StatefulBuilder(
          builder: (context, setState) {
            ...
            if (!initialized) {
              initialized = true;
              Future.microtask(() => fetchInitialData(setState));
            }
                    ...
                    child: Column(
                      children: [
                        ...
                        Container(
                          alignment: Alignment.centerLeft,
                          child: Text(
                            selectedId?.toString() ?? "Loading ID...",
                          ),
                        ),
                        ...
                        ElevatedButton(
                          onPressed: isSaving
                            ...
                            await JblApiService.addRecutPcsIssue(
                              ...
                              woNumber: selectedWo!,
                              component: selectedComponent!,
                              cutWidth: double.tryParse(
                                cutWidthCtrl.text,
                              ) ??
                                  0,
                              cutLength: double.tryParse(
                                cutLengthCtrl.text,
                              ) ??
                                  0,
                              cutSizeQty: double.tryParse(
                                cutSizeQtyCtrl.text,
                              ) ??
                                  0,
                              netWt:
                              double.tryParse(netWtCtrl.text) ?? 0,
                            );
