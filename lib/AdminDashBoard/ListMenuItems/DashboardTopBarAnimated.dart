...
// ─────────────────────────────────────────────
//  PULL-TO-REFRESH WIRING (do this in the parent screen that owns the
//  CustomScrollView):
//
//  final _headerKey = GlobalKey<DashboardTopBarAnimatedState>();
//
//  RefreshIndicator(
//    onRefresh: () => _headerKey.currentState?.refresh() ?? Future.value(),
...
class DashboardTopBarAnimated extends StatefulWidget {
  ...
  State<DashboardTopBarAnimated> createState() =>
      DashboardTopBarAnimatedState();
}

class DashboardTopBarAnimatedState extends State<DashboardTopBarAnimated>
  ...
  /// Public entry point so a `RefreshIndicator` (or anything else) higher
  /// up the tree can trigger a fresh API pull via a GlobalKey. See the
  /// wiring comment at the top of this file.
...
// ─────────────────────────────────────────────
//  SLIVER DELEGATE — makes the top bar collapse as the page scrolls, and
//  forwards a `headerKey` so a parent RefreshIndicator can call
//  `headerKey.currentState?.refresh()` on pull-to-refresh.
// ─────────────────────────────────────────────
class DashboardCollapsingHeaderDelegate extends SliverPersistentHeaderDelegate {
  final String unit;
  final String fromDate;
  final String toDate;
  final GlobalKey<DashboardTopBarAnimatedState>? headerKey;
